...
const Navs = ({ history, getFiatRate }) => {
  console.log('Navs', history)
  return (
    <>
      <IconButton
        onClick={async () =>
          history.push({
            ...
            state: { fiatRate: await getFiatRate({ symbol: 'ETH' }) }
          })
        }
        aria-label='Approval'
        isRound
...
export const TradeInputOld = ({ history }: RouterProps) => {
  const {
    control,
    handleSubmit,
    getValues,
  ...
  const getGetFiatRate = async isERC20 => {
    return await getFiatRate({ symbol: isERC20 ? 'ETH' : sellAsset.currency.symbol })
  }
    ...
    <SlideTransition>
      <Box as='form' onSubmit={handleSubmit(onSubmit)} mb={2}>
        ...
        >
          ...
          />
          {/* <Navs history={history} getFiatRate={getGetFiatRate} /> */}
